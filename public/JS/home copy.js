document.addEventListener("DOMContentLoaded", async () => {
  ...
  const updateLessonStatus = async (lessonId, status, progress, score) => {
    try {
      const response = await fetch("/home/updatelesson", {
        ...
        body: JSON.stringify({ lessonId, status, progress, score }),
      });
  ...
  function showQuizResults() {
    ...
    const scoreFraction = `${correctAnswers}/${totalQuestions}`;
    ...
    updateLessonStatus(currentLessonId, status, progress, scoreFraction);

    // Display the quiz results
    quizContainer.innerHTML = `
      <div style="text-align: center; color: #1e6ea2; font-family: Arial, sans-serif; margin-top: 20vh;">
        <h1 style="font-size: 4rem; font-weight: bold; margin-bottom: 20px;">${scoreFraction}</h1>
