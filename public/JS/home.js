document.addEventListener("DOMContentLoaded", async () => {
  ...
  const API_BASE_URL = window.location.origin; // Use the current domain
  const fetchLessons = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/home/lessons`);
  ...
  const fetchLessonDetails = async (lessonId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/home/progress/${lessonId}`);
      if (!response.ok) throw new Error("Failed to fetch lesson details.");
      const details = await response.json();
      console.log("Lesson Details:", details); // Debugging
      return details;
    } catch (error) {
  ...
  const createPopup = (details) => {
    const formatCompletedAt = (completedAt) => {
      if (!completedAt) return "Not Available";

      const date = new Date(completedAt); // Parse the timestamp
      const options = {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
      ...
      return date.toLocaleString("en-GB", options); // Format as "DD/MM/YYYY HH:mm"
    };

    const popup = document.createElement("div");
    popup.classList.add("lesson-popup");
    ...
    popup.innerHTML = `
      ...
      <p><strong>Completed At:</strong> ${formatCompletedAt(
  ...
  const renderLessons = async () => {
    try {
      ...
      const sortedUnitNames = Object.keys(units).sort((a, b) => {
        const aNumber = parseInt(a.match(/\d+/), 10) || 0;
      ...
      sortedUnitNames.forEach((unitName, index) => {
        ...
        units[unitName].lessons.forEach((lesson) => {
          ...
          // Add the lesson details and 3-dots icon
          lessonDiv.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: space-between;">
                  ...
                  ${
                    lesson.status === "completed"
                      ? `<img src="/assets/icons/blue/Icons-VnEng_completed-icon.png" 
                           alt="Completed" 
                           style="width: 24px; height: 24px; margin-left: 5px; position: relative; top: 5px; object-fit: contain;">`
                      : ""
                  }
                  ${
                    lesson.status !== "completed"
              ...
              <img 
                src="/assets/icons/blue/dot.png" 
                alt="Details Icon" 
                class="details-icon"
                style="width: 30px; height: 30px; cursor: pointer; margin-left: 10px; object-fit: contain;"
          ...
          // Add event listener for the 3-dots icon
          const detailsIcon = lessonDiv.querySelector(".details-icon");
          detailsIcon.addEventListener("click", async (event) => {
            ...
            const existingPopup = document.querySelector(".lesson-popup");

            // If the popup exists and is for the same lesson, remove it
            if (existingPopup) {
              const existingLessonId =
                existingPopup.getAttribute("data-lesson-id");
              if (existingLessonId === lessonId) {
                existingPopup.remove();
                return; // Exit early since we're toggling off the popup
              } else {
                // Remove the existing popup if it's for a different lesson
                existingPopup.remove();
              }
            }
            ...
            if (!details) {
              console.error("No details available for this lesson.");
              return;
            }

            // Create and show the popup
            const popup = createPopup(details);
            popup.setAttribute("data-lesson-id", lessonId); // Store the lessonId in the popup for future checks
            ...
            popup.style.left = `${
              rect.left + window.scrollX - popup.offsetWidth / 2
            }px`;
  ...
  const updateLessonStatus = async (
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/home/updatelesson`, {
        method: "POST",
        headers: {
  ...
  function showQuizResults() {
    ...
    exitButton.addEventListener("click", () => {
      ...
      window.location.href = `${API_BASE_URL}/HTML/home.html`;
    });
  }
 ...
 `;
  ...
  const fetchUserLessons = async (userId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/user-lessons/${userId}`);
